...
backend_target_str = ["ModelInferBackend.CPU", "ModelInferBackend.CUDA", "ModelInferBackend.APPLE"]

parser = argparse.ArgumentParser("Benchmarks for Custom Models.")
...
parser.add_argument("--not_print_header", action="store_true", help="Not print header")
...
if __name__ == '__main__':
    ...
    if args.cfg is not None:
        ...
        for cfg_fname in sorted(os.listdir("custom_config")):
            skip_flag = False
            for exc in excludes:
            ...
            with open(os.path.join("custom_config", cfg_fname), "r") as f:
                cfg = yaml.safe_load(f)
            cfgs.append(cfg)
    else:
    ...
    if args.all:
        ...
        backend_str = backend_target_str[backend_target_id]
        if not args.not_print_header:
    ...
    for cfg in cfgs:
        ...
        if args.cfg_overwrite_backend_target >= 0:
            backend_id = backend_target[args.cfg_overwrite_backend_target]
            benchmark.setBackend(backend_id)

        # Instantiate model
        model_config = cfg['Model']
        model_handler, model_paths = CustomModels.get(model_config.pop('name'))
