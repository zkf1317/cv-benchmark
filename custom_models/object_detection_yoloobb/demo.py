...
from custom_models.object_detection_yoloobb.tools import draw_detections
...
from yoloobbdet import YoloObbDet
...
if __name__=='__main__':
    parser = argparse.ArgumentParser(description='Yoloobb inference')
    parser.add_argument('--input', '-i', type=str,
                        help='Path to the input image. Omit for using default camera.')
    parser.add_argument('--model', '-m', type=str, default='yolo11x-obb-ir9.onnx',
    ...
    model_net = YoloObbDet(model_path= args.model,
                      class_names=classes,
    ...
    if args.input is not None:
        ...
        # Inference
        tm.start()
        boxes = model_net.infer(image)
        tm.stop()
        print("Inference time: {:.2f} ms".format(tm.getTimeMilli()))

        img = draw_detections(image, boxes, classes)
